...
public:
  ...
  Quat operator ^ (const double alpha) {
      Cvec3 v = Cvec3(q_[1], q_[2], q_[3]);
      if (norm2(v) < CS175_EPS2) return *this;
      double sine = norm(v);
      double cosine = q_[0];
      double theta = atan2(sine, cosine);
      Cvec3 k = v.normalize();
      return Quat(cos(alpha * theta), k*sin(alpha * theta));
  }

  bool operator == (Quat a) {
      if (q_[0] != a.q_[0]) return false;
      if (q_[1] != a.q_[1]) return false;
      if (q_[2] != a.q_[2]) return false;
      if (q_[3] != a.q_[3]) return false;
      return true;
  }
